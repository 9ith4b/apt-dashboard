...
- Observable 检索、本地与AI上下文富化、自动 Indicator，以及 Campaign 时间线
...
通过 `http://<服务器地址>:8180` 访问系统。首次登录后请立即修改管理员密码，并在“AI 自动化”中添加、测试并选择默认模型，然后启用“全量AI处理”“无人值守运营”“自动生成确认事件”和“AI自动维护 Indicator”。低置信度、证据缺口和归因冲突会保留异常与审计记录，但不会等待人工放行；人工修改过的报告字段和 Indicator 不会被后续AI覆盖。首次启用会自动让启用前已采集、曾被过滤或曾处理失败的材料重新进入 AI 队列；需要重试或补齐旧数据的AI IOC判断时，可再次点击“历史材料回填”。
