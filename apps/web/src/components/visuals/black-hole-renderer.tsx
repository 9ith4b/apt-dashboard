...
type HalfFloatTextureExtension = {
...
export function BlackHoleHeroSection({
  ...
  useEffect(() => {
    ...
    const renderer = dbg
      ...
      : ""
    const software =
      /swiftshader|llvmpipe|softpipe|software|microsoft basic/i.test(renderer)
    const isGL2 =
    ...
    if (isGL2) {
      ...
      const hf = gl.getExtension(
        "OES_texture_half_float"
      ) as HalfFloatTextureExtension | null
    ...
    function resize() {
      ...
      const dpr = software
        ...
        : Math.min(
            ...
            Math.max(1, props.current.maxDpr)
          )
    ...
    function render(t: number) {
      ...
      let rx = fz,
        ry = 0,
        rz = -fx // cross(fwd, worldUp), worldUp = +y
      ...
      gl!.uniform1f(
        compProg.u.uScrimAmt!,
        Math.max(0, Math.min(1, C.scrimStrength))
      )
